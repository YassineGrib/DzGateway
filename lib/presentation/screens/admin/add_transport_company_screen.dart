...
class AddTransportCompanyScreen extends StatefulWidget {
  final String? companyId;

  const AddTransportCompanyScreen({super.key, this.companyId});

  @override
  State<AddTransportCompanyScreen> createState() => _AddTransportCompanyScreenState();
}

class _AddTransportCompanyScreenState extends State<AddTransportCompanyScreen> {
  ...
  TransportCompany? _currentCompany;
  ...
  Future<void> _loadCompanyData() async {
    ...
    try {
      final company = await TransportService().getTransportCompanyById(widget.companyId!);
      if (company != null) {
        setState(() {
          ...
          _selectedTransportType = TransportType.fromString(company.transportType);
  ...
  Future<void> _saveCompany() async {
    ...
    try {
      if (_isEditing && _currentCompany != null) {
        final companyData = TransportCompany(
          id: _currentCompany!.id,
          name: _nameController.text.trim(),
          phone: _phoneController.text.trim().isEmpty ? null : _phoneController.text.trim(),
          transportType: _selectedTransportType?.value ?? 'bus',
        ...
        await TransportService().updateTransportCompany(companyData);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
        ...
        await TransportService().createTransportCompany(companyData);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
