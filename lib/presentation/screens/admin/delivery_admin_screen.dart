...
class _DeliveryAdminScreenState extends State<DeliveryAdminScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('خطأ في تحميل شركات التوصيل: ${e.toString()}'),
  ...
  List<DeliveryCompany> get _filteredCompanies {
    if (_searchQuery.isEmpty) {
      return _companies;
    }
    return _companies.where((company) {
      return company.name.toLowerCase().contains(_searchQuery.toLowerCase()) ||
          (company.deliveryTime?.toLowerCase().contains(_searchQuery.toLowerCase()) ?? false) ||
  ...
  Future<void> _deleteCompany(String companyId) async {
    ...
    if (confirmed == true) {
      try {
        // Note: You'll need to implement deleteDeliveryCompany in DeliveryService
        // await DeliveryService.deleteDeliveryCompany(companyId);
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'البحث في شركات التوصيل...',
  ...
  void _showAddEditCompanyDialog({DeliveryCompany? company}) {
    showDialog(
      context: context,
      builder: (context) => AddEditDeliveryCompanyDialog(
        company: company,
        onSaved: () {
          _loadCompanies();
        },
...
class AddEditDeliveryCompanyDialog extends StatefulWidget {
  final DeliveryCompany? company;
  final VoidCallback onSaved;

  const AddEditDeliveryCompanyDialog({
    super.key,
    this.company,
  ...
  @override
  State<AddEditDeliveryCompanyDialog> createState() => _AddEditDeliveryCompanyDialogState();
}

class _AddEditDeliveryCompanyDialogState extends State<AddEditDeliveryCompanyDialog> {
  ...
  void initState() {
    ...
    _serviceTypeController = TextEditingController(text: widget.company?.deliveryTime ?? '');
    _addressController = TextEditingController(text: widget.company?.description ?? '');
    _phoneController = TextEditingController(text: widget.company?.phone ?? '');
    _emailController = TextEditingController(text: widget.company?.name ?? '');
    _deliveryTimeController = TextEditingController(text: widget.company?.deliveryTime ?? '');
  ...
  Future<void> _saveCompany() async {
    ...
    try {
      // Note: You'll need to implement createDeliveryCompany and updateDeliveryCompany in DeliveryService
      /*
      if (widget.company == null) {
        // Create new company
        await DeliveryService.createDeliveryCompany(
        ...
        await DeliveryService.updateDeliveryCompany(
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Text(
              widget.company == null ? 'إضافة شركة توصيل جديدة' : 'تعديل شركة التوصيل',
              style: const TextStyle(
                fontSize: 20,
                fontWeight: FontWeight.bold,
              ),
